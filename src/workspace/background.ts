...
export class BackgroundConfiguration {
	...
	private static setBackgroundAllImageObject(value: string[]): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			this.queueSet(
				() => Promise.resolve(this.setBackgroundConfiguration('allImageCodes', result)),
				resolve,
				reject,
			);
		});
	...
	static async setBackgroundAllImageCodes(value: string | number, state: 'add' | 'delete' = 'add'): Promise<void> {
		...
		await this.setBackgroundAllImageObject(list)
			.then(() => {
				return Promise.resolve();
			})
			.catch(err => {
	...
	static check() {
		...
		// 插入队列执行
		this.queueSet(() => Promise.resolve(this.setBackgroundConfiguration('allImageCodes', result)));
	}
