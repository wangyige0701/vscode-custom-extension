...
import { backgroundHashCodes } from "../../../app-background-cache";
import { matchWYGFileName } from "../../../app-background-files";
...
export function checkImageFiles (files: [string, FileType][], uri: Uri): Promise<BufferAndCodeMergeType[]> {
    return new Promise((resolve, reject) => {
        ...
        for (const i of range(files.length)) {
            ...
            const index = backgroundHashCodes.indexOf(reg[1]);
