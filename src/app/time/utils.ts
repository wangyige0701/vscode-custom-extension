	...
	if (cycle === CycleItem.DAY) {
		...
		if (!cycle.includes(w)) {
			...
			if (w < cycle[0]) {
				...
				return timestamp + 1000 * 60 * 60 * 24 * (6 - w + cycle[0] + 1);
			} else {
...
export function getTimeString(twelve: boolean = true, icon: boolean = true) {
	const result: number[] = [],
		iconPrefix = icon ? '$(wangyige-clock) ' : '';
	...
	return twelve
		? function (timestamp: number) {
				...
				return `${iconPrefix}${y}/${_a(M)}/${_a(d)} 周${weeksName[day]}$(chevron-left)${_a(h)}:${_a(
					m,
				)} ${meridiem}$(chevron-right)`;
			}
		: function (timestamp: number) {
				_getDate(timestamp);
...
export function changeHourTo24(time: string, symbol: string = ':') {
	...
	// 判断12小时制还是24小时制
	let period =
		time.endsWith('p') || time.endsWith('P') ? 'pm' : time.endsWith('a') || time.endsWith('A') ? 'am' : false;
